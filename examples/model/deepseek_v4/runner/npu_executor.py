...
def _find_pypto_lib_deepseek_v4_dir(pypto_root: str | None = None) -> Path:
    ...
    candidate = _FALLBACK_PYPTO_LIB_ROOT / "models" / "deepseek" / "v4"
    if candidate.is_dir():
        return candidate
...
class DeepSeekV4PyptoExecutor(CorePyptoExecutor):
    ...
    def lookup_embeddings(self, model: RuntimeModel, token_ids: torch.Tensor) -> torch.Tensor:
        ...
        return embeddings.reshape(*token_ids.shape, model.config.hidden_size)
    ...
    def _load_kernel_modules(self) -> dict[str, object]:
