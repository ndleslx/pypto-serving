...
def test_deepseek_compile_builds_one_runtime_scalar_layer_callable(tmp_path, monkeypatch):
    ...
    assert prefill_args[prefill_order.index("cmp_kv")].shape == (8, 43 * 64, 128, 1, 512)
    ...
    assert prefill_args[prefill_order.index("hca_cmp_kv_state")].shape == (8, 20 * 1024, 8, 512)
    assert prefill_args[prefill_order.index("csa_cmp_kv_state")].shape == (8, 21 * 2048, 4, 1024)
    assert prefill_args[prefill_order.index("csa_inner_kv_state")].shape == (8, 21 * 2048, 4, 256)
    assert prefill_args[prefill_order.index("hca_compress_state_block_table")].shape == (8, 1024)
    assert prefill_args[prefill_order.index("csa_compress_state_block_table")].shape == (8, 2048)
    assert prefill_args[prefill_order.index("csa_inner_compress_state_block_table")].shape == (8, 2048)
    assert prefill_args[prefill_order.index("ori_block_table")].shape == (8, 1)
    assert prefill_args[prefill_order.index("cmp_block_table")].shape == (8, 8)
...
def test_deepseek_prepare_prefill_inputs_maps_chunk_and_sparse_tables():
    ...
    assert prepared.cmp_block_table.shape == (8, 8)
...
def test_deepseek_prepare_decode_inputs_uses_scratch_slots_for_fixed_rows():
    ...
    assert prepared.cmp_block_table.shape == (8, 32, 8)
    assert prepared.ori_slot_mapping[0, :6].tolist() == [126, 127, 131, 132, 382, 383]
    assert prepared.hca_cmp_slot_mapping[0, :6].tolist() == [-1, 0, -1, -1, -1, 2048]
    assert prepared.csa_cmp_slot_mapping[0, :6].tolist() == [-1, 31, 1024, -1, -1, 2079]
